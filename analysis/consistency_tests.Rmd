...
usair_g <- simplify(usair_g, remove.multiple = T, 
                    edge.attr.comb = list(weight = "sum", "ignore"))

usair_raw <- get.edgelist(usair_g) %>% 
    data.frame(stringsAsFactors = F) %>% 
    mutate(weights = E(usair_g)$weight) %>% 
    uncount(weights)
```
...
# 8. AMLC institutions collaboration
...
```{r}
amlc_inst_raw <- read.csv(url('https://zenodo.org/record/437549/files/amlc_collaboration_network.csv'), stringsAsFactors = F) %>% 
    select(3, 4) %>% 
    filter(institution2 != '')

amlc_inst_g <- amlc_inst_raw %>% 
    graph.data.frame(directed = FALSE)

E(amlc_inst_g)$weight <- 1

E(amlc_inst_g)$weight <- rep(1,length(E(amlc_inst_g)))

amlc_inst_g <- simplify(amlc_inst_g, remove.multiple = T, edge.attr.comb = 'sum')
```
...
Pij_tit_dis_b <- calc_Pmatrix(tit_g, 100, 'fastgreedy', permutations = F, boot.data = tit_raw)
