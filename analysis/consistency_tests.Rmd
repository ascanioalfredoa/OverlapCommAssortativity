...
usair_g <- simplify(usair_g, remove.multiple = T, edge.attr.comb = list(weight="sum", "ignore"))
```
...
```{r}
amlc_inst_raw <- read.csv(url('https://zenodo.org/record/437549/files/amlc_collaboration_network.csv')) %>% 
    select(3, 4) %>% 
    filter(institution2 != '')
...
# 9.AMLC coun collaboration

Ramírez-Ramírez, Rubén D, Montilla, Luis Miguel, Cavada-Blanco, Francoise, & Cróquer, Aldo. (2017). Identification of strengths and weaknesses of cooperative efforts within the wider Caribbean using a network approach [dataset] [Data set]. Zenodo. http://doi.org/10.5281/zenodo.437549

```{r}
amlc_coun_raw <- read.csv(url('https://zenodo.org/record/437549/files/amlc_collaboration_network.csv')) %>% 
    select(5, 6) %>% 
    filter(country2 != '')
